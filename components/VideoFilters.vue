    ...
    <template #body>
      <div class="space-y-1.5 text-sm">
        ...
        <div v-if="filters.length === 0" class="text-center py-8 text-gray-500 text-xs">
          <Icon name="ph:camera" size="32px" class="mb-2 opacity-30" />
          ...
          <div v-if="filter.enabled && hasParams(filter.type)" class="filter-params">
            <template v-for="(spec, paramName) in getFilterParams(filter.type)" :key="paramName">
              ...
              <div v-else class="param-row">
                <span class="param-label" :class="{ 'w-6': false }">
...
const _ownCtx = props.ctx ? null : useVideoFilters(filterOpts.value);
...
const { getFilterLabel, getCategoryLabel, getFilterParams, hasParams, getFiltersForCategory, getDefaultStep, formatValue } = useFilterPresentation(FILTER_TYPES, FILTER_CATEGORIES);
...
.param-value {
  @apply text-[10px] text-violet-400/80 w-14 text-right tabular-nums font-mono;
}
.bypass-btn {
