<template>
  ...
  <Dialog v-model:visible="isOpen" :modal="true" class="input-pane-dialog">
    <template #header>
      <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">Add Input</h3>
    </template>
      ...
      <TabPanels>
        <TabPanel v-for="(item, index) in types" :key="item.label" :value="index">
          <div class="p-2 bg-gray-100 dark:bg-gray-700 rounded-lg">
            <h2 class="text-base font-bold mb-1 text-gray-800 dark:text-gray-200">{{ item.fields['type'].label }}</h2>
            <p class="text-xs italic mb-2 text-gray-500 dark:text-gray-400">{{ item.fields['type'].description }}</p>
            <form @submit.prevent="submitCreateInput(item.label)" class="space-y-2">
              ...
              <div class="space-y-2">
                <div>
                  ...
                  <span class="text-xs text-gray-600 dark:text-gray-400">Volume: {{ Math.round(formData[item.label].volume * 100) }}%</span>
                </div>
